...
%      unrecognized property name or invalid value makes property applicationc
...
% Last Modified by GUIDE v2.5 24-Oct-2024 11:13:59
...
%code to setup plot labels
 ...
 if get(handles.toggleMeasSource,'Value')== get(handles.toggleMeasSource,'Min')
    ...
    set(handles.getMeas,'String', '555.555.555.555');
...
if get(handles.savingResult,'Value')== get(handles.savingResult,'Max')
    savingResults(readings, stabArray, tau, handles, sPeriod,maxCI,minCI,begintime); %saving results
end
...
function savingResults(RawFrequency, DeviationResult, DeviationTau, handles, SamplingInterval, DeviationResultMaxCI, DeviationResultMinCI,BeginTimeInFilename)
    folder = get(handles.savingFolder,'String');
    name = sprintf('%s_%s.mat', datestr(BeginTimeInFilename,'yyyy_mm_dd_HH_MM_SS'), get(handles.savingFilename,'String'));
    fullpath = fullfile(folder, name);

    if get(handles.listDev,'Value') == 1
        DeviationType = 'Allan Deviation';
    else
        DeviationType = 'Hadamard Deviation';
    end

    save(fullpath,"RawFrequency","DeviationType","DeviationResult","DeviationTau","SamplingInterval","DeviationResultMaxCI","DeviationResultMinCI");
...
period = str2double(get(handles.enterSamplePeriod,'String'));
total = str2double(get(handles.measCount,'String'));

set(handles.totalTime,'String', string(period * total));
...
enterSamplePeriod_Callback(hObject, eventdata, handles);
end
...
% --- Executes on button press in savingResult.
function savingResult_Callback(hObject, eventdata, handles)
% hObject    handle to savingResult (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of savingResult
end
...
% Hints: get(hObject,'String') returns contents of savingFilename as text
%        str2double(get(hObject,'String')) returns contents of savingFilename as a double
end
